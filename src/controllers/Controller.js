class Controller {
    ...
    try {
      ...
      if (updatedRegister !== null) {
        return res.status(200).json({
          message: `${this.entityName} width id ${id} updated with success!`,
        });
      } else {
        return res
          .status(404)
  ...
  async delete(req, res) {
    ...
    try {
      const registerToDelete = await this.entityService.deleteRegister(id);
      if (registerToDelete !== null) {
        return res.status(200).json({
          message: `${this.entityName} width id ${id} deleted with success!`,
        });
      } else {
        return res
          .status(404)
